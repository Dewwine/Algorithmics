...
void RBTree::preorderBST(Node *&ptr)
{
    ...
    cout << ptr->data << "    -   " << (ptr->color == BLACK ? "BLACK" : "RED") << endl;
...
void RBTree::SHOW_MAIN(Node *&tree, int n)
{
    if (tree == NULL)
    ...
    SHOW_MAIN(tree->left, n - 3);  // Обошли левое поддерево
    SHOW_MAIN(tree->right, n + 3); // Обошли правое поддерево
}
